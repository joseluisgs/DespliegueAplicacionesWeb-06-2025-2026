...
- [Bloque 6: Validación de Interfaz con Cypress](#bloque-6-validación-de-interfaz-con-cypress)
...
### Bloque 6: Validación de Interfaz con Cypress

61. Una diferencia clave de Cypress frente a Selenium es que Cypress:
    a) Corre fuera del navegador.
    b) Ejecuta los tests dentro del navegador.
    c) Es mucho más lento.
    d) No soporta JavaScript.

62. ¿Qué característica de Cypress permite revisar cada comando ejecutado en el pasado?
    a) Back to the Future.
    b) Time Travel.
    c) History Brush.
    d) Snapshot Manager.

63. Según las mejores prácticas de Cypress, ¿cuál es el mejor **selector**?
    a) ID (#user-form).
    b) Clases CSS (.btn-success).
    c) Atributos personalizados como `data-cy`.
    d) Etiquetas HTML (button).

64. El comando para escribir texto en un campo de entrada en Cypress es:
    a) `cy.write()`
    b) `cy.type()`
    c) `cy.input()`
    d) `cy.fill()`

65. La aserción `.should()` en Cypress se diferencia de `.then()` porque:
    a) `.should()` reintenta automáticamente hasta que pase o agote el timeout.
    b) `.should()` solo se ejecuta una vez.
    c) `.then()` es más rápida.
    d) No hay ninguna diferencia.

66. Para simular un clic en un botón con el texto "Enviar", usaríamos:
    a) `cy.click('Enviar')`
    b) `cy.get('button').contains('Enviar').click()`
    c) `cy.send('Enviar')`
    d) `cy.push('Enviar')`

67. Cypress permite ejecutar tests en modo **headless**, lo que significa:
    a) Sin conexión a internet.
    b) Sin interfaz gráfica (ideal para CI/CD).
    c) Sin código fuente.
    d) Sin usar el teclado.

68. ¿Qué herramienta se usa en Cypress para interceptar y validar llamadas a APIs?
    a) `cy.api()`
    b) `cy.intercept()`
    c) `cy.request()`
    d) `cy.mock()`

69. El archivo donde se configuran los **Comandos Personalizados** en Cypress es:
    a) `cypress.config.js`
    b) `support/commands.js`
    c) `e2e/custom.js`
    d) `fixtures/example.json`

70. ¿Qué ventaja ofrece Cypress respecto a las esperas (`waits`)?
    a) Hay que poner `sleep(5000)` en cada línea.
    b) Gestiona esperas automáticas e inteligentes.
    c) No permite esperar nunca.
    d) El programador debe calcular el tiempo de red manualmente.

71. En un entorno Docker, ¿por qué es útil Cypress?
    a) Para que la web cargue más rápido.
    b) Para ejecutar pruebas E2E en un entorno aislado y reproducible.
    c) Para minar criptomonedas.
    d) Para sustituir al servidor web.

72. ¿Qué hace el comando `cy.visit('/')`?
    a) Cierra la aplicación.
    b) Navega a la URL base configurada.
    c) Visita al equipo de desarrollo.
    d) Borra las cookies.

73. La arquitectura de Cypress permite tomar screenshots y videos automáticamente cuando:
    a) El test pasa.
    b) El test falla.
    c) El usuario hace scroll.
    d) Siempre, cada 2 segundos.

74. Para verificar que un elemento **no** existe en el DOM, se usa:
    a) `should('not.exist')`
    b) `should('be.invisible')`
    c) `expect(null)`
    d) `assert.missing()`

75. ¿Qué framework de aserciones utiliza Cypress bajo el capó?
    a) JUnit.
    b) Chai.
    c) NUnit.
    d) Mockito.
